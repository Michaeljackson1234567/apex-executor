/* APEX EXECUTOR — Renderer (Native WebView2 Build) */

// ══ WebView2 IPC Bridge ══════════════════════════════════════════════════════
// Replaces Electron's preload.js — sends JSON messages to C# MainForm
const _pendingCalls = {};
let _callId = 0;
...
function callNative(action, data) {
  return new Promise((resolve) => {
    ...
    window.chrome.webview.postMessage(JSON.stringify({ action, id, data: data || null }));
    // Safety timeout — 3 minutes for inject, 30s for everything else
    const ms = action === 'inject' ? 180000 : 30000;
    setTimeout(() => {
      if (_pendingCalls[id]) {
...
function sendNative(action) {
  window.chrome.webview.postMessage(JSON.stringify({ action, id: '', data: null }));
}
...
window.chrome.webview.addEventListener('message', (event) => {
  try {
    ...
    const id = msg.id;
    if (id && _pendingCalls[id]) {
      _pendingCalls[id](msg.result);
      delete _pendingCalls[id];
    }
  } catch (e) { console.error('Message parse error:', e); }
});

// ══ Compatibility layer — same API as old preload.js ══════════════════════════
const apex = {
  minimize:       () => sendNative('win-minimize'),
  maximize:       () => sendNative('win-maximize'),
  close:          () => sendNative('win-close'),
  inject:         () => callNative('inject'),
  executeScript:  (s) => callNative('execute-script', { script: s }),
  checkRoblox:    () => callNative('check-roblox'),
  getRobloxUser:  () => callNative('get-roblox-user'),
  searchScripts:  (q, p) => callNative('search-scripts', { query: q, page: p }),
  fetchScripts:   (p) => callNative('fetch-scripts', { page: p }),
  fetchRawScript: (u) => callNative('fetch-raw-script', { url: u }),
  saveScript:     (name, code) => callNative('save-script', { name, code }),
  getSavedScripts:() => callNative('get-saved-scripts'),
  deleteScript:   (f) => callNative('delete-script', { filename: f }),
  openScriptsFolder: () => callNative('open-scripts-folder'),
  setTopmost:     (v) => callNative('set-topmost', { value: v }),
  setOpacity:     (v) => callNative('set-opacity', { value: v }),
  getVersion:     () => callNative('get-version'),
  checkForUpdate: () => callNative('check-for-update'),
  getPendingChangelog: () => callNative('get-pending-changelog'),
  markChangelogSeen:  () => callNative('mark-changelog-seen'),
  syncUpdate:     () => Promise.resolve({ success: false }),
  restartApp:     () => {},
  onUpdateProgress: () => {},
};

// ══════════════════════════════════════════════════════════════════════════════
// Everything below is the same renderer logic, now using the `apex` object above
// ══════════════════════════════════════════════════════════════════════════════
...
log('info', 'Apex Executor v2.0 — Native Build initialized.');
...
async function doInject() {
  ...
  showOv('INJECTING...');
  log('info', '▸ Attaching to Roblox (direct engine call)...');
  await checkRoblox();
...
require(['vs/editor/editor.main'], function () {
  ...
  monaco.languages.setMonarchTokensProvider('lua', {
    ...
    tokenizer: {
      root: [
        [/--\[([=]*)\[/, 'comment', '@blockComment.$1'],[/--.*$/, 'comment'],
        [/"/, 'string', '@stringDouble'],[/'/, 'string', '@stringSingle'],[/\[([=]*)\[/, 'string', '@blockString.$1'],
        [/0[xX][0-9a-fA-F]*/, 'number.hex'],[/\d+(\.\d+)?([eE][-+]?\d+)?/, 'number'],[/\.\d+([eE][-+]?\d+)?/, 'number'],
        [/[a-zA-Z_]\w*/, { cases: {
      ...
      blockComment: [[/\]([=]*)\]/, { cases: { '$1==$S2': { token: 'comment', next: '@pop' }, '@default': 'comment' } }],[/./, 'comment']],
      stringDouble: [[/[^\\"]+/, 'string'],[/\\./, 'string.escape'],[/"/, 'string', '@pop']],
      stringSingle: [[/[^\\']+/, 'string'],[/\\./, 'string.escape'],[/'/, 'string', '@pop']],
      blockString: [[/\]([=]*)\]/, { cases: { '$1==$S2': { token: 'string', next: '@pop' }, '@default': 'string' } }],[/./, 'string']],
    },
  });
  ...
  monaco.languages.registerCompletionItemProvider('lua', {
    provideCompletionItems: () => ({
      ...
      ].map(label => ({ label, kind: monaco.languages.CompletionItemKind.Snippet, insertText: label, range: null }))
    })
  });
  ...
  monaco.editor.defineTheme('apex-dark', {
    ...
    rules: [
      ...
      { token: 'keyword.true', foreground: 'f59e0b' },{ token: 'keyword.false', foreground: 'f59e0b' },{ token: 'keyword.nil', foreground: 'f59e0b' },
      { token: 'keyword.function', foreground: 'c084fc' },{ token: 'keyword.local', foreground: 'c084fc' },{ token: 'keyword.return', foreground: 'c084fc' },
      { token: 'keyword.if', foreground: 'c084fc' },{ token: 'keyword.then', foreground: 'c084fc' },{ token: 'keyword.else', foreground: 'c084fc' },
      { token: 'keyword.elseif', foreground: 'c084fc' },{ token: 'keyword.end', foreground: 'c084fc' },{ token: 'keyword.for', foreground: 'c084fc' },
      { token: 'keyword.while', foreground: 'c084fc' },{ token: 'keyword.do', foreground: 'c084fc' },{ token: 'keyword.repeat', foreground: 'c084fc' },
      { token: 'keyword.until', foreground: 'c084fc' },{ token: 'keyword.in', foreground: 'c084fc' },{ token: 'keyword.and', foreground: 'c084fc' },
      { token: 'keyword.or', foreground: 'c084fc' },{ token: 'keyword.not', foreground: 'c084fc' },{ token: 'keyword.break', foreground: 'c084fc' },
      { token: 'keyword.goto', foreground: 'c084fc' },
      { token: 'support.function', foreground: '60a5fa' },{ token: 'support.other', foreground: '38bdf8' },{ token: 'type', foreground: '2dd4bf' },
      { token: 'variable.language', foreground: 'fb923c' },{ token: 'string', foreground: '4ade80' },{ token: 'string.escape', foreground: 'fbbf24' },
      { token: 'number', foreground: 'fb923c' },{ token: 'number.hex', foreground: 'fb923c' },{ token: 'operator', foreground: '94a3b8' },
      { token: 'delimiter', foreground: '64748b' },{ token: 'delimiter.curly', foreground: 'fbbf24' },{ token: 'delimiter.square', foreground: 'c084fc' },
      { token: 'delimiter.parenthesis', foreground: '94a3b8' },{ token: 'identifier', foreground: 'cbd5e1' },
    ],
    colors: {
      'editor.background': '#0a0a0c', 'editor.foreground': '#cbd5e1',
      'editor.lineHighlightBackground': '#ffffff06', 'editor.selectionBackground': '#ffffff14',
      'editor.inactiveSelectionBackground': '#ffffff08', 'editorLineNumber.foreground': '#27272a',
      'editorLineNumber.activeForeground': '#71717a', 'editorCursor.foreground': '#ffffff',
      'editorWhitespace.foreground': '#1e1e24', 'editorIndentGuide.background1': '#1e1e24',
      'editorIndentGuide.activeBackground1': '#3f3f46', 'editorWidget.background': '#111114',
      'editorWidget.border': '#27272a', 'editorSuggestWidget.background': '#111114',
      'editorSuggestWidget.selectedBackground': '#1e1e24', 'scrollbarSlider.background': '#27272a60',
  ...
  editor = monaco.editor.create(document.getElementById('monaco-wrap'), {
    value: [
      '-- ══════════════════════════════════════════',
      '-- ║       APEX EXECUTOR  v2.0 NATIVE       ║',
      '-- ══════════════════════════════════════════',
      '', '-- 1. Click INJECT on Home tab (Roblox must be open)',
      '-- 2. Write your Lua script here', '-- 3. Click Execute or Ctrl+Enter', '',
    ...
    language: 'lua', theme: 'apex-dark', fontSize: 13,
    fontFamily: '"JetBrains Mono", "Cascadia Code", monospace', fontLigatures: true, lineHeight: 22,
    minimap: { enabled: true, side: 'right', renderCharacters: false },
    scrollBeyondLastLine: false, wordWrap: 'off', renderLineHighlight: 'gutter',
    cursorStyle: 'line', cursorBlinking: 'phase', smoothScrolling: true, mouseWheelZoom: true,
    bracketPairColorization: { enabled: true }, guides: { bracketPairs: true, indentation: true },
    overviewRulerBorder: false, padding: { top: 12, bottom: 12 }, automaticLayout: true,
...
document.getElementById('st-theme').onchange = (e) => {
  ...
  if (editor) monaco.editor.setTheme(isLight ? 'vs' : 'apex-dark');
...
function applyFilter() {
  const cards = document.querySelectorAll('#sh-grid .sc');
  cards.forEach(card => {
    const data = JSON.parse(card.dataset.scriptInfo || '{}');
    let show = true;
    switch (activeFilter) {
      case 'free': show = !data.paid; break;
      case 'paid': show = !!data.paid; break;
      case 'key': show = !!data.keySystem; break;
      case 'keyless': show = !data.keySystem; break;
      case 'mobile': show = !!data.mobileReady; break;
      case 'pc': show = !data.mobileReady; break;
      default: show = true;
    }
...
function dedup(scripts) {
  const seen = new Set();
  return scripts.filter(s => { const key = (s.title || '').trim().toLowerCase(); if (seen.has(key)) return false; seen.add(key); return true; });
}
...
function mkCard(s, i) {
  ...
  d.dataset.scriptInfo = JSON.stringify({ keySystem: s.keySystem, mobileReady: s.mobileReady, paid: s.paid });
  const img = getImg(s);
  const game = s.game ? s.game.title : 'Unknown';
  let bgs = '';
  if (s.keySystem) bgs += '<span class="bg bg-k">🔑</span>';
  else bgs += '<span class="bg bg-v" style="background:rgba(34,197,94,.12);color:#22c55e">🔓</span>';
  if (s.mobileReady) bgs += '<span class="bg bg-u">📱</span>';
  else bgs += '<span class="bg" style="background:rgba(59,130,246,.12);color:#3b82f6;font-size:8px;font-weight:700;padding:2px 5px;border-radius:3px">💻</span>';
...
async function openModal(s) {
  ...
  const game = s.game ? s.game.title : 'Unknown';
  document.getElementById('modal-meta').innerHTML = `<span>🎮 ${esc(game)}</span><span>👁 ${(s.views||0).toLocaleString()} views</span>`;
  const bb = document.getElementById('modal-badges');
  bb.innerHTML = '';
  if (s.keySystem) bb.innerHTML += '<span class="bg bg-k">🔑 Key</span>';
  else bb.innerHTML += '<span class="bg bg-v">🔓 Keyless</span>';
  if (s.mobileReady) bb.innerHTML += '<span class="bg bg-u">📱 Mobile</span>';
  else bb.innerHTML += '<span class="bg" style="background:rgba(59,130,246,.12);color:#3b82f6;font-size:8px;font-weight:700;padding:2px 5px;border-radius:3px">💻 PC Only</span>';
  if (s.paid) bb.innerHTML += '<span class="bg bg-p">Paid</span>';
  else bb.innerHTML += '<span class="bg bg-v">Free</span>';

  document.getElementById('modal-code').textContent = 'Loading script...';
...
(async function showPendingChangelog() {
  try {
    ...
    if (cl.success && cl.changelog && cl.changelog.length > 0) {
      document.getElementById('changelog-ver').textContent = 'v' + cl.version;
      const list = document.getElementById('changelog-list');
      list.innerHTML = '';
      cl.changelog.forEach(item => { const li = document.createElement('li'); li.textContent = item; list.appendChild(li); });
